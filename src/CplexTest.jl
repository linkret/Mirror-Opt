...
function build_pairs(A, ups)
    ...
    for t in LETTERS
            ...
            for e in external_edges(U)
                ...
                if all(in_bounds, L)
                    ...
                    if !(key in seen)
                        ...
                        if matches_example || total_sum >= 5 # 5 is default, 0 would be ideal
                            push!(plist, (
                                cellsU = cu,
                                cellsL = cl,
                                weight = total_sum,
        ...
        # Remove duplicates by exact inflated cellset, largest Weighted # EDIT: has basically 0 effect
        plist = sort(plist, by = x -> (x.bbox, -x.weight)) # sort by bbox for speed
...
# of the chosen pairs.
function pick_greedy!(pairs::Dict{Symbol, Vector{NamedTuple}}, randomize = false, first_letter::Symbol = :W, first_index::Union{Int,Nothing} = nothing)
    ...
    outer_edge((x,y)) = (min(x, y) <= 2 || max(x, y) >= W-1)
    ...
    function best_feasible_for(t::Symbol)
        # TODO: dont always pick the best one
    ...
    # Select first letter placement either by fixed index or by metric
    t = first_letter
    if first_index !== nothing
        v = get(pairs, t, NamedTuple[])
        if isempty(v)
            error("No placements available for letter $(t)")
        end
        idx = clamp(first_index, 1, length(v))
        if idx != first_index
            println("Requested index ", first_index, " out of range for ", t, "; clamped to ", idx)
        end
        pr = v[idx]
        best_t = t
        best_pr = pr
        best_w = pr.weight / max(1, taken_cost(pr))
        println("Fixed-first pick: letter ", t, ", index ", idx, ", weight ", pr.weight)
    else
        # used to be for loop that tries all letters; now just for the chosen one
...
function solve_pentomino(A, optimize, randomize, first_letter::Symbol = :W, first_index::Union{Int,Nothing} = nothing)
    ups   = enumerate_uppercase_placements()
    pairs = build_pairs(A, ups)
    picked = pick_greedy!(pairs, randomize, first_letter, first_index)
    ...
    try
        ...
        # set_optimizer_attribute(model, "objective_bound", 180.0)
        # set_silent(model)
        ...
        for pid in eachindex(P)
           ...
           # M = length(qs)
           if !isempty(qs)
               @constraint(model, M * y[pid] + sum(y[q] for q in qs) ≤ M)
           end
        end

        # Per-pair “outer neighbor cells” (small-M flattened to pairwise)
        # This is a replacement for the constraints above with M - many more constraints of much simpler structure. Should be equivalent
        ...
        #     for rid in qs
        #         if pid < rid
        #             @constraint(model, y[pid] + y[rid] <= 1)
        #         end
        #     end
        # end
...
function run_greedy(optimize = true, randomize = false, first_letter::Symbol = :W, first_index::Union{Int,Nothing} = nothing)
    status, best, P, chosen = solve_pentomino(A, optimize, randomize, first_letter, first_index)
